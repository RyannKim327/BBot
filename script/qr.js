...
module.exports = (api, body, event) => {
	if(event.type == "message_reply"){
		...
		if(xpl.length > 0){
			...
			api.sendMessage(`QR Code Command:\nThis command contains of 2 features, the first one is by executing the command in message reply, and the other one is by using this format in normal message:\nNoBhie: qr <query>.`, event.threadID, event.messageID)
		}
	}
}
